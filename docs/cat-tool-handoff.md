...
1. **入稿工作區**：頂部**語系配對列**（來源→目標，14 語系，未配對或相同語系時兩條入稿路徑全部封鎖、按鈕 disabled）。兩張入稿卡：
   ...
   - xlsx 拖放：前綴欄位（前綴-分頁名）+ 拖放區（兩段式：解析→摘要預覽→建立）。表頭依所選語系代碼定位欄位（大小寫不敏感）+「標號」欄存 srcNo；ja/zh-tw 皆空的模板列略過；語系配對變更即作廢暫存。**表頭雙欄必備**（V35 後續修正）：來源與目標語系欄缺一即擋該分頁；全部分頁都不符時彈出置中 Modal「語系配對與檔案不符」，列出所選配對與表頭偵測到的語系欄（防止選錯配對時無聲匯入、文件掛錯語系標籤；純原文稿請走貼上入稿）
...
- **情境列**（單卡整合）：目前檔案名 +「← 返回專案管理區」「⟲ 重置確認狀態」+ 句段整理 5 icon 鈕（編輯/分割、重新排列、合併、新增、刪除；icon-only 附 title 提示）+ 翻譯/校對雙進度條（朱紅/teal 淺色 60% 透明，即時連動）
- **句段整理五功能（V36，仿 Termsoup Modal 式，共用外殼 segToolOverlay：720px 加寬 Modal + 可捲動句段清單 + 取消/送出）**：
  - **編輯／分割句子**：每句一個 textarea（autoGrow）可直接改原文；游標放句內按 Enter 於游標處分割（`isComposing`/keyCode 229 守門，IME 組字 Enter 不觸發；頭尾分割忽略）。分割後譯文全留前半句、後半句空白無 srcNo；**原文被改動＝退回未確認、tmId 斷開**（V28 規則的原文側延伸，已與使用者定案）；清空的句子送出後移除
  - **排序句子**：原生 HTML5 拖曳（dragstart/dragover 中點判斷 insertBefore/dragend），送出後依 DOM 順序重排陣列。**不動任何句段內容與狀態**；復原快照以 segId 對回、仍有效不作廢
  - **合併句子**：多選＋相鄰檢核（不相鄰報「連續的句子才可合併」）；原文/譯文各依語系串接（langJoiner：ja/zh/ko/th 不加空格、其餘半形空格），**譯文串接不作廢**（Termsoup 做法，已定案）；合併句退未確認、tmId 斷開；被併句的 TM 紀錄保留
  - **新增句子**：清單單選插入位置（首項「插入為第一句」，其餘＝插在該句之後）＋下方 textarea 輸入新句原文；新句譯文空白、未確認、無 tmId/srcNo；不影響既有句段，復原快照不作廢
  - **刪除句子**：多選刪除；「已有譯文」「已確認」句段顯示朱紅警示標籤；TM 紀錄保留（同「刪文件不動 TM」原則）
...
- ~~句段合併與交換~~ ~~拆分句段~~ → **V36 已全數實作**（句段整理四功能，見翻譯工作區功能明細）。原分析中「合併不可逆」的顧慮因分割功能同輪落地而解除；srcNo 代價照舊（分割後半句/被併句 srcNo 遺失，影響未來模板寫回，已接受）
